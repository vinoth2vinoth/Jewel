...
export function runAudit(cwd: string = process.cwd()): void {
  console.log('Running Jewel Repository Safety and Quality Audit...\n');

  const checks: AuditCheck[] = [];

  // Helper to push checks
  const addCheck = (id: string, title: string, status: 'PASS' | 'WARN' | 'FAIL', details: string) => {
  ...
  if (initialized && config) {
    ...
    const hasCmds = Object.values(cmds).some((v: any) => v && v.trim() !== '');
    if (hasCmds) {
  ...
  const lookForTests = (dir: string): boolean => {
    if (!fs.existsSync(dir)) return false;
    const list = fs.readdirSync(dir);
    for (const f of list) {
      if (['node_modules', '.git', '.jewel'].includes(f)) continue;
      const fullPath = path.join(dir, f);
      const stat = fs.statSync(fullPath);
      if (stat.isDirectory()) {
        if (f.toLowerCase() === 'test' || f.toLowerCase() === 'tests') return true;
        if (lookForTests(fullPath)) return true;
      } else {
        if (/\.(test|spec)\.[a-zA-Z0-9]+$/.test(f)) return true;
      }
    }
    return false;
  };
