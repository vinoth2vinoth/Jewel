  ...
  try {
    switch (command) {
      ...
      case 'ship': {
        ...
        runShip({ sessionId, branch, message, cwd: process.cwd() });
        break;
      }
      ...
      case 'continue': {
        ...
        for (let i = argStart + (feedback ? 1 : 0); i < args.length; i++) {
          ...
          else if (arg === '--provider') { providerOverride = args[++i]; }
          else if (arg === '--model') { modelOverride = args[++i]; }
          else if (arg === '--temperature') { temperatureOverride = parseFloat(args[++i]); }
          else if (arg === '--max-output-tokens') { maxOutputTokensOverride = parseInt(args[++i], 10); }
          else if (!feedback && !arg.startsWith('-')) { feedback = arg; }
        }
