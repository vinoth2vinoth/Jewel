...
class OpenRouterAdapter {
    ...
    async callLLM(messages, config, method, sessionPath) {
        ...
        const { capabilities, isKnown, warning } = (0, model_capabilities_1.getModelCapabilities)('openrouter', model);
        if (warning) {
      ...
      If you still wish to proceed without structured output enforcement, set "allowUnstructuredProviderFallback" to true in your configuration (warning: this may reduce reliability).`);
        ...
        const normalized = (0, response_normalizer_1.normalizeResponse)(data, 'openrouter', model);
